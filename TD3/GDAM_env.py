...
class ImplementEnv:
    def __init__(self, args):
        ...
        self.vel_pub_rob = rospy.Publisher('/r1/cmd_vel', Twist, queue_size=1)
        ...
        self.navOdom = rospy.Subscriber('/r1/odom', Odometry, self.Odom_callback, queue_size=1)
    ...
    def step(self, act):
        ...
        laser_in = np.array(v.ranges[:])
        laser_state= np.array(
        [min(laser_in[0:17]), min(laser_in[18:35]),
        min(laser_in[36:53]), min(laser_in[54:71]),
        min(laser_in[72:89]), min(laser_in[90:107]),
        min(laser_in[108:125]), min(laser_in[126:143]),
        min(laser_in[144:161]), min(laser_in[162:179]),
        min(laser_in[180:197]), min(laser_in[198:215]),
        min(laser_in[216:233]), min(laser_in[234:251]),
        min(laser_in[252:269]), min(laser_in[270:287]), 
        min(laser_in[288:305]), min(laser_in[306:323]),
        min(laser_in[324:341]), min(laser_in[342:359])])
        ...
        if global_goal_distance < 1.5:
            ...
            self.global_goal_x = 4.0
            self.global_goal_y = 8.0
            self.nodes = deque(maxlen=400)
            self.nodes.append([self.global_goal_x, self.global_goal_y, self.global_goal_x, self.global_goal_y, 0])
